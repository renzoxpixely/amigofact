...
class DocumentController extends Controller
{
    ...
    public function searchLots(Request $request)
    {
        ...
        if ($request->document_item_id) {
            ...
            $selected = $request->selected === 'true' ? true : false;
            $lots = $request->lots ?? [];
            ...
            if(!$selected){
                $records
                ->where(function($q) use ($request, $warehouse){
                    $q->where('item_id', $request->item_id)->where('has_sale', false)
                    ->where('warehouse_id', $warehouse->id);
                })->orWhere(function($q) use($lots){
                    if($lots && is_array($lots)) $q->whereIn('id', $lots);
                })
                ->latest();
            }else{
                $records
                ->where(function($q) use($request, $lots, $warehouse){
                    if($lots && is_array($lots)) { $q->whereIn('id', $lots);}
                    else  {$q->where('item_id', $request->item_id)->where('has_sale', false)
                    ->where('warehouse_id', $warehouse->id);}
                })->latest();
            }
        }
