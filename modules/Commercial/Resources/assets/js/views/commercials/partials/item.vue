        ...
        <form autocomplete="off"
            ...
            <div class="form-body">
                <div class="row">
                    ...
                    <div class="col-md-5">
                        <div :class="{'has-danger': errors.hall_id}"
                             class="form-group">
                            <label class="control-label">Sala</label>
                            <el-select v-model="form.hall_id" @change="changeHall">
                                <el-option v-for="hall in halls" :key="hall.id" :value="hall.id" :label="hall.name">{{ hall.name }}</el-option>
                            </el-select>
                            <small v-if="errors.hall_id"
                                   class="form-control-feedback"
                                   v-text="errors.hall_id[0]"></small>
                        </div>
                    </div>
                    ...
                    <div class="col-md-12 mt-2">
                        <div class="form-group">
                            <label class="control-label">Subproductos</label>
                            <table class="table">
                                <thead>
                                <tr>
                                    <th class="font-weight-bold" width="5%">#</th>
                                    <th class="font-weight-bold" width="30%">Descripción</th>
                                    <th class="font-weight-bold">Precio Unit.</th>
                                    <th class="font-weight-bold">Cantidad</th>
                                </tr>
                                </thead>
                                <tbody>
                                <tr v-for="(row, index) in form.sub_items"
                                    :key="index">
                                    <td>{{ index + 1 }}</td>
                                    <td>{{ row.item.description }}</td>
                                    <td>
                                        <el-input v-model="row.unit_price">
											<template v-if="row.item.currency_type_symbol" slot="prepend">
												{{ row.item.currency_type_symbol }}
											</template>
										</el-input>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <button class="btn waves-effect waves-light btn-xs btn-danger"
                                                type="button"
                                                @click.prevent="clickRemoveSubItem(index)"><i class="fas fa-trash"></i>
                                        </button>
                                    </td>
                                </tr>
                                </tbody>
            ...
            <div class="form-actions text-right pt-2">
                ...
                <el-button v-if="form.item_id"
                           native-type="submit"
                           type="primary">{{ isEdit ? 'Editar' : 'Agregar' }}
                </el-button>
            </div>
        </form>
...
export default {
    ...
    components: {itemForm, WarehousesDetail, 'vue-ckeditor': VueCkeditor.component, SelectLotsForm},
    ...
    methods: {
        ...
        // },
        parseForm(form_data, item) {

            form_data.item  = {... item};
            this.lots = [];//form_data.item.lots
			form_data.item.lots = [];
            form_data.unit_price = form_data.item.sale_unit_price;
            form_data.has_igv = form_data.item.has_igv;
            form_data.affectation_igv_type_id = form_data.item.sale_affectation_igv_type_id;
            form_data.quantity = 1;

            form_data.sub_items = [];
            if(form_data.item.sub_items){
                form_data.item.sub_items.forEach(sub_item => {
					let sub_form  = this.paramForm();
					sub_form.item_id = sub_item.id;
                    form_data.sub_items.push(this.parseForm(sub_form, sub_item));
                });

            }
            delete form_data.item.sub_items
        ...
        changeItem() {
            ...
            this.form = this.parseForm(this.form, _.find(this.items, {'id': this.form.item_id}));
        ...
        changeHall(){
            const hall = _.find(this.halls, {id: this.form.hall_id});
            this.form.hall = hall ? hall : null
        },
        selectedPrice(row) {
            let valor = 0
        ...
        clickAddItem() {
			...
			if(this.validHall && this.form.hall_id == null) {
                this.errors.hall_id = ["La Sala es requerido"]; return;
            }

            let select_lots = _.filter(this.form.item.lots, {'has_sale': true})
            if (this.form.item.series_enabled) {
                if (select_lots.length != this.form.quantity)
                    this.$message.success('Se agregaron '+select_lots.length+' series de '+this.form.quantity+' máquinas.');
			...
			this.row.hall_id = this.form.hall_id;
			this.row.hall = this.form.hall;

			/**SUBITEM */
			this.row.sub_items = [];
            this.form.sub_items.forEach(sub_item => {
				let unit_price = (this.form.has_igv) ? sub_item.unit_price : sub_item.unit_price * 1.18;
				// this.form.item.unit_price = this.form.unit_price
				sub_item.unit_price = unit_price;
				sub_item.item.unit_price = unit_price;
				sub_item.item.presentation = this.form.item.presentation;
				sub_item.affectation_igv_type = this.form.affectation_igv_type;

                let sub_row = calculateRowItem(sub_item, this.currencyTypeIdActive, this.exchangeRateSale);
				sub_row.hall_id = this.form.hall_id;
				sub_row.hall = this.form.hall;
                this.row.sub_items.push(sub_row);
            });
			/**END SUBITEM */
            ...
            const index = this.rowData ? this.rowData.index : null

            this.$emit('add', this.row, index);
            this.setFocusSelectItem()

            if(index !== null) this.close()
        },
        focusSelectItem() {
        ...
        create() {
            ...
            this.isEdit = this.rowData ? true: false
            this.titleDialog = this.isEdit ? 'Editar Producto' : 'Agregar Producto'

            if(this.rowData){
                const row = this.rowData.form
                this.form.item_id = row.item_id
                this.changeItem()

                this.form.affectation_igv_type_id = row.affectation_igv_type_id
                this.form.quantity = row.quantity
                this.form.hall_id = row.hall_id
                this.form.unit_price = (row.item.has_igv) ? row.item.unit_price : _.round(row.item.unit_price / 1.18, 2)
                this.changeHall()


                this.addRowSelectLot(row.item.lots)

                for(let idx=0;idx<this.form.sub_items.length;idx++){
                    const index = row.sub_items.findIndex(s1=> s1.item_id === this.form.sub_items[idx].item_id)
                    if(index !== -1){
                        const select = row.sub_items[index]
                        this.form.sub_items[idx].unit_price = (select.item.has_igv) ? select.item.unit_price : _.round(select.item.unit_price / 1.18, 2)
                        this.form.sub_items[idx].quantity = select.quantity
                    }else{
                        this.form.sub_items.splice(idx, 1);
                    }
                }

                // row.sub_items.forEach(sub_item => {
                //     const index = this.form.sub_items.findIndex(s1=> s1.item_id === sub_item.item_id);
                //     if(index !== -1){
                //         this.form.sub_items[index].unit_price = (sub_item.item.has_igv) ? sub_item.item.unit_price : _.round(sub_item.item.unit_price / 1.18, 2)
                //         this.form.sub_items[index].quantity = sub_item.quantity
                //     }
                // })
            }
            //     this.initializeFields()
        ...
        addRowSelectLot(lots) {
            this.lots = Array.from(lots)
			this.form.item.lots = Array.from(lots)
        },
		clickRemoveSubItem(index) {
            this.form.sub_items.splice(index, 1);
        },
    }
}
