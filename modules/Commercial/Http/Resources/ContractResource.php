...
class ContractResource extends JsonResource
{
    ...
    {
        $contract = Contract::with(['contract_type' => function($q){
			$q->select('id', 'name');
		}, 'items.lots.item_lot'])->where('id', $this->id)->get()->transform(function($row, $key){
            return self::getTransformContract($row);
        })->first();
    ...
    public static function getTransformContract($row){
		$items = self::getTransformContractItem($row['items']);
		$machines = self::getTransformItemsMachines($items);
        $contract_type_name = $row['contract_type']['name'] ?? null;
        $row['items'] =  $items;
		$row['machines'] =  $machines;
        $row['is_rental'] = $contract_type_name == ContractType::RENTAL;
        $row['is_stake'] = $contract_type_name == ContractType::STAKE;
        return $row;
    }

    public static function getTransformContractItem($rows){
        return $rows->transform(function($row, $key){ 
            ...
            $row['lots'] = self::getTransformContractItemLot($lots);
    ...
    public static function getTransformContractItemLot($rows){
        return $rows->transform(function($row, $key){ 
            return [
                'id' => $row->id,
