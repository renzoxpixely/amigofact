<template>
    <div>
        ...
        >
            ...
            <invoice-items v-if="form.contract" :items="form.contract.items"></invoice-items>
            <div  v-show="document.total > 0" class="col-lg-12">
                <div class="form-group pull-right">
                    <label class="control-label"> Total </label> <br>
                    <label class="control-label">{{ document.currency_type_id }} {{ this.document.total }}</label>
                </div>
...
import InvoiceItems from "./invoice_items.vue";
import moment from "moment";
import {mapActions, mapState} from "vuex/dist/vuex.mjs";

export default {
    components: {DocumentOptions, SaleNoteOptions, InvoiceItems},
    ...
    methods: {
        ...
        clickAddPayment() {
            ...
            if(
                ...
                this.form.contract.total !== null
            ){
                total =this.form.contract.total
            }
        ...
        getCustomer() {
            this.$http
                .get(
                    `/${this.resource_documents}/search/customer/${this.form.contract.customer_id}`
                )
                .then((response) => {
                    this.customers = response.data.customers;
                    this.document.customer_id = this.form.contract.customer_id;
                    this.changeCustomer();
                });
        },
        changeCustomer() {
        ...
        assignDocument() {
            ...
            this.document.contract_id = this.form.id;
        ...
        async getRecord(){
            ...
            await this.$http
                        ...
                        .then((response) => {
                            ...
                            this.form.contract_id = this.recordId;
                            this.document.payments =
                                response.data.data.contract.payments;
                            this.document.total = this.form.contract.total;
                            this.document.currency_type_id = this.form.contract.currency_type_id;
                            this.document.payment_condition_id = this.form.contract.payment_condition_id;
        ...
        clickSendEmail() {
            ...
            this.$http
                .post(`/${this.resource}/email`, {
                    ...
                    id: this.form.id,
                    customer_id: this.form.contract.customer_id,
                })
                .then((response) => {
                    if (response.data.success) {
        ...
        async validateQuantityandSeries() {
            let error = 0;
            await this.form.contract.items.forEach((element) => {
