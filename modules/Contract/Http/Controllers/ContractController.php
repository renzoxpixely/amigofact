...
class ContractController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('contract::contracts.index', compact('configuration'));
    }
    ...
    public function view($id = null)
    {
        ...
        return view('contract::contracts.view', compact('id', 'quotationId', 'showPayments'));
    }
...
public function getContratosItems()
    ...
    private function getRecords($request){
        ...
        switch($request->column) {
            ...
            case 'trade_name':
                return Contract::where('customer_id', '=', $request->value);
            case 'contract_type':
                return Contract::where('contract_type_id', '=', $request->value);
        }
    }
    ...
    public function store(ContractRequest $request) {
        ...
        DB::connection('tenant')->transaction(function () use ($request) {
            ...
            foreach ($contract_items as $contract_item){
                ...
                $lots =  ContractItem::with(['lots'])->where('id', $contract_item->id)->first()->lots()->get()->transform(function($row){
                    return $row->item_lot_id;
                });
                $lots = $lots ? $lots->toArray() : [];
                dd($lots);
                if(count($lots) > 0) ItemLot::whereIn('id', $lots)->update(['has_sale' => 0]);
                ContractItem::with(['lots'])->where('id', $contract_item->id)->first()->lots()->delete();
            }
