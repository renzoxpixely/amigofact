...
class InventoryChangeServiceProvider extends ServiceProvider
{
    ...
    private function createdItem()
    {
        ...
        Item::created(function ($item) {
            ...
            if(!$item->is_set){
                $this->createInitialInventory($item->id, $item->stock, $item->warehouse_id ?: $warehouse->id);
            }else{
                $item_warehouse = ItemWarehouse::firstOrNew(['item_id' => $item->id, 'warehouse_id' => $item->warehouse_id]);
