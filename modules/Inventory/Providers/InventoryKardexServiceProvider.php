...
class InventoryKardexServiceProvider extends ServiceProvider
{
    ...
    private function purchase() {
        PurchaseItem::created(function (PurchaseItem $purchase_item) {
            ...
            //Actualiza Valor de Costo Unitario
            $purchase = $purchase_item->purchase;
            $total_import_cost_without_igv = $purchase->total_cost + $purchase->total_tax_rights + $purchase->total_tax_perception;
            $this->updateUnitCostValue($purchase_item->item_id, $purchase_item->unit_price, $purchase_item->purchase->total, $total_import_cost_without_igv);

            //Actualiza Precio Unitario
            if($purchase_item->prorated) $this->updateSaleUnitPrice($purchase_item->item_id);

        });
    }

    private function item_lot() {
        ItemLot::created(function(ItemLot $item_lot) {
            //Actualiza Valor de Costo Unitario
            $purchase_item = $item_lot->item_loteable()->first();
            $class_name = get_class($purchase_item);
            if($class_name == PurchaseItem::class){
                $purchase = $purchase_item->purchase;
                $total_import_cost_without_igv = $purchase->total_cost + $purchase->total_tax_rights + $purchase->total_tax_perception;
                $this->updateUnitCostValueItemLot($item_lot->id, $purchase_item->unit_price, $purchase_item->purchase->total, $total_import_cost_without_igv, $purchase_item->purchase->currency_type_id);
            }
        });
    }
