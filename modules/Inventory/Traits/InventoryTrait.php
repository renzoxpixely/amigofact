...
//                $this->updateStock($item->id, $item->stock, $warehouse->id);
     ...
     *
     */
    public function updateSaleUnitPrice($item_id)
    {
        $sum = 0;
        $items = PurchaseItem::select('unit_price')->where('prorated', true)->where('item_id', $item_id)->get();
        foreach($items as $item){
            $sum += $item->unit_price;
        }
        if($items->count() > 0) $sum = $sum/$items->count();

        $item = Item::find($item_id);
        $item->sale_unit_price = number_format($sum, 4, '.','');
        $item->save();
    }
     ...
     *
     */
    public function updateUnitCostValue($item_id, $unit_price, $total, $total_import_cost_without_igv)
    {
        $unit_cost_value = ($total_import_cost_without_igv / $total) * $unit_price;
        $item = Item::find($item_id);
        $item->unit_cost_value = number_format($unit_cost_value, 2, '.','');
        $item->save();
    }

    public function updateUnitCostValueItemLot($id, $unit_price, $total, $total_import_cost_without_igv, $currency_type_id)
    {
        $unit_cost_value = ($total_import_cost_without_igv / $total) * $unit_price;
        $item = ItemLot::find($id);
        $item->unit_cost_value = number_format($unit_cost_value, 2, '.','');
        $item->currency_type_id = $currency_type_id;
        $item->save();
    }

    public function createInventoryTransfer($contract, $item_id, $quantity, $warehouse_id, $warehouse_destination_id, $lots = [])
    {
        ...
        $inventory->inventories_transfer_id = null;
        $inventory->save();

        foreach ($lots as $lot) {

            $item_lot = ItemLot::find($lot->item_lot_id);
            if($item_lot){
                $item_lot->has_sale = 1;
                $item_lot->warehouse_id = $warehouse_destination_id;
                $item_lot->save();
            }
        }
    }
