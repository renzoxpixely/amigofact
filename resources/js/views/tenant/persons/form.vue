                                    ...
                                    <table class="table table-sm mb-0">
                                        ...
                                        <tr v-for="(row, index) in form.person_halls"
                                            ...
                                            <template v-else>
                                                ...
                                                <td :class="{'has-danger': errors['person_halls.'+index+'.department_id']}">
                                                    <el-select v-model="row.department_id"
                                                            dusk="department_id"
                                                            filterable
                                                            @change="filterProvinceRoom(row)">
                                                        <el-option v-for="option in all_departments"
                                                                :key="option.id"
                                                                :label="option.description"
                                                                :value="option.id"></el-option>
                                                    </el-select>
													<small  v-if="errors['person_halls.'+index+'.department_id']"
															class="form-control-feedback"
															v-text="errors['person_halls.'+index+'.department_id'][0]"></small>
                                                </td>
                                                <td :class="{'has-danger': errors['person_halls.'+index+'.province_id']}">
                                                    <el-select v-model="row.province_id"
                                                            dusk="province_id"
                                                            filterable
                                                            @change="filterDistrictRoom(row)">
                                                        <el-option v-for="option in row.provinces"
                                                                :key="option.id"
                                                                :label="option.description"
                                                                :value="option.id"></el-option>
                                                    </el-select>
													<small  v-if="errors['person_halls.'+index+'.province_id']"
															class="form-control-feedback"
															v-text="errors['person_halls.'+index+'.province_id'][0]"></small>
                                                </td>
                                                <td :class="{'has-danger': errors['person_halls.'+index+'.district_id']}">
                                                    <el-select v-model="row.district_id"
                                                            dusk="district_id"
                                                            filterable>
                                                        <el-option v-for="option in row.districts"
                                                                :key="option.id"
                                                                :label="option.description"
                                                                :value="option.id"></el-option>
                                                    </el-select>
													<small  v-if="errors['person_halls.'+index+'.district_id']"
															class="form-control-feedback"
															v-text="errors['person_halls.'+index+'.district_id'][0]"></small>
                                                </td>
                                                <td :class="{'has-danger': errors['person_halls.'+index+'.address']}">
                                                    <el-input v-model="row.address"></el-input>
													<small  v-if="errors['person_halls.'+index+'.address']"
															class="form-control-feedback"
															v-text="errors['person_halls.'+index+'.address'][0]"></small>
                                                </td>
                                            <template v-if="row.id">
					...
					<el-tab-pane class
                        ...
                        <div class="row ">
							...
							<div class="col">
                                <a class="control-label font-weight-bold text-info"
                                href="#"
                                @click="clickUser"> [ + {{ form.user.email == null ? 'Agregar' : 'Editar' }}]</a>
                            </div>
						</div>
					</el-tab-pane>
                    <el-tab-pane class
...
export default {
    ...
    async created() {
        ...
        this.$eventHub.$on('reloadDataUsers', (form) => {
			if(form){
				this.form.user_id = form.id
				this.form.user.name = form.name
				this.form.user.email = form.email
			}
        })
...
//                this.form.addresses[0].telephone = data.telefono;
        ...
        filterProvinceRoom(row){
			row.province_id = null
            row.district_id = null
            this.filterProvincesRoom(row);
        },
        filterProvincesRoom(row){
            row.provinces = this.all_provinces.filter(f => {
                return f.department_id === row.department_id
            })
        },
        filterDistrictRoom(row){
			row.district_id = null
            this.filterDistrictsRoom(row);
        },
        filterDistrictsRoom(row){
            row.districts = this.all_districts.filter(f => {
                return f.province_id === row.province_id
            })
        },
        clickAddRow() {
			...
			delete this.errors['person_halls.'+index+'.department_id'];
			delete this.errors['person_halls.'+index+'.province_id'];
			delete this.errors['person_halls.'+index+'.district_id'];
			delete this.errors['person_halls.'+index+'.address'];
        },
        clickCancel(index) {
		...
		filterSelectHalls() {
			if(!this.form.person_halls) return false;
			this.form.person_halls.forEach( row => {
				if(row.department_id) this.filterProvincesRoom(row);
				if(row.province_id) this.filterDistrictsRoom(row);
			})
		},
		clickUser() {
			this.$emit('clickUser', this.form.user_id);
		}
    }
}
</script>
