<template>
    <el-dialog :close-on-click-modal="false"
            ...
            <el-tabs v-model="activeName">
                <el-tab-pane class
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <div :class="{'has-danger': errors.mincetur_game_code}"
                                 class="form-group">
                                <label class="control-label">Código Mincetur</label>
                                <el-input v-model="form.mincetur_game_code"
                                          dusk="item_code"></el-input>
                                <small v-if="errors.mincetur_game_code"
                                       class="form-control-feedback"
                                       v-text="errors.item_code[0]"></small>
                            </div>
                        </div>
                ...
                <el-tab-pane v-if="!isSubItem" name="seven">
                    <span slot="label">Subproductos</span>
                    <div class="row">
                        <div v-show="form.unit_type_id !='ZZ'"
                             class="col-md-12">
                                        ...
                                        <tr v-for="(row, index) in form.sub_items"
                                            ...
                                            <td class="series-table-actions text-right">
                                                <button class="btn waves-effect waves-light btn-xs btn-primary"
                                                        type="button"
                                                        @click.prevent="clickEditSubItem(row.id, index)">
                                                    <i class="fa fa-edit"></i>
                                                </button>
                                                <button class="btn waves-effect waves-light btn-xs btn-danger"
                                                        type="button"
                                                        @click.prevent="clickRemoveSubItem(row.id, index)">
                        ...
                        <div class="col mt-2">
                            <a class="control-label font-weight-bold text-info"
                               href="#"
                               @click="clickAddSubItem"> [ + Agregar]</a>
                        </div>
                    </div>
                </el-tab-pane>
                <el-tab-pane class
                ...
                <el-tab-pane name="six">
                    <span slot="label">Depreciación</span>
                    <div class="row">
                        <div class="col-md-3">
                            <label class="control-label">% Depreciación anual</label>
                            <div class="form-group">
                                <el-input-number v-model="form.annual_depreciation" :min="0.01" :precision="2" :max="99"></el-input-number>
                        ...
                        <table class="table" style="width:auto;">
                            <thead>
								<tr>
									...
									<th class="font-weight-bold">Fecha Fin</th>
									<th class="font-weight-bold">P.U de compra</th>
									<th class="font-weight-bold">Dep. Acumulada</th>
								</tr>
                            </thead>
                            <tbody>
								<tr>
                ...
                <el-tab-pane name="eight" v-if="showSeries">
                    <span slot="label">Series</span>
                    <lots-form
                        :lots="this.form.lots"
                        :canAddRows="false"
        ...
        <el-dialog :title="'Agregar series'" width="50%"  :visible="showDialogLots" :close-on-click-modal="false" :close-on-press-escape="false" append-to-body :show-close="false">
            <lots-form
                :lots="form.lots"
                :canAddRows="true"
                :recordId="recordId"
                :stock="form.stock"
                @addRowLot="addRowLot"
                @closeDialog="showDialogLots = false"
            >
            </lots-form>
        </el-dialog>
        ...
        <items-form
            :recordId="recordSubItemId"
            :showDialog.sync="showDialogSubItem"
            :type="type"
            :isSubItem="true"
            :parentId="recordId"
            :width="'60%'"
...
export default {
	name: "ItemsForm",
    props: {showDialog: {required: false}, recordId: {required: false}, external:{required: false}, type:{required: false},pharmacy : {required: false},width : {default: '65%'}, isSubItem: {default: false}, parentId: {default: null}},
    mixins: [deletable],
	components: {
        LotsForm,
        ExtraInfo
    ...
    async created() {
        ...
        await this.$http.get(`/${this.resource}/tables`)
            .then(response => {
                ...
                this.gameNames = data.game_names
    ...
    methods: {
        ...
        clickAddSubItem() {
            this.recordSubItemId = null
            this.indexSubItem = null
            this.showDialogSubItem = true
        },
        savedItem(data){
            if(this.indexSubItem == null){
                this.form.sub_items.push({
                    id: data.id,
                    description: data.description,
                    unit_type_id: data.unit_type_id
                });
            }else{
                this.form.sub_items[this.indexSubItem].description = data.description
                this.form.sub_items[this.indexSubItem].unit_type_id = data.unit_type_id
            }

        },
        clickEditSubItem(id, index) {
            this.recordSubItemId = id
            this.indexSubItem = index
            this.showDialogSubItem = true
        },
        clickRemoveSubItem(id, index){
        ...
        async create() {
            ...
            const itemTitle = this.isSubItem ? 'SubProducto' : 'Producto';
            this.titleDialog = (this.recordId) ? `Editar ${itemTitle}` : `Nuevo ${itemTitle}`
            ...
            this.form.is_sub_item = this.isSubItem
            ...
            await this.$http.post(`/${this.resource}`, this.form)
                .then(response => {
                    if (response.data.success) {
                        ...
                        if(this.isSubItem){
                            this.$emit('savedItem', response.data)
        ...
        async addRdFile(aFile, aFilename, someLotIds) {
            const formData = new FormData;

            formData.append('rd_file', aFile);
            formData.append('rd_filename', aFilename);
            someLotIds.forEach(id => formData.append('rd_lot_ids[]', id));


            this.$http.post(`/${this.resource}/rd-file`, formData).then(
                response => {
                    this.$message.success('Archivo RD agregado con éxito')

                    this.form.lots = response.data;
                }
            ).catch(
                error => {
                    if (error.response.status === 422) {
                        for (const prop in error.response.data) {
                            this.$message.error(error.response.data[prop][0]);
                        }
