<template>
    <div>
                ...
                <div class="row">
                    <div class="col-lg-12 col-md-6 d-flex align-items-end mt-4">
                        <div class="form-group">
                            ...
                            <button v-if="addedInvoices.length"
                                    class="btn waves-effect waves-light btn-primary"
                                    type="button"
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                ...
                                <tr v-for="(row, index) in addedInvoices"
                                    ...
                                    <td>{{ translateInvoiceType(row.invoice_type) }}</td>
                                    <td>{{ row.title}}</td>
                                    ...
                                    <td class="text-right">
                                        <button class="btn waves-effect waves-light btn-xs btn-info"
                                                type="button"
                                                @click.prevent="editInvoice(row.id)">Editar
                                        </button>
        ...
        <purchase-edit-dialog
            :resourceId="form.purchase_order_id"
...
export default {
    props: ['external', 'recordDataPurchase',],
    components: {
        PurchaseEditDialog,
    ...
    methods: {
        ...
        addFiles(files, fileList) {
            const mappedFiles = fileList.map(file => file.raw);
        ...
        async submit() {
            ...
            this.form.form_data.append('arrival_date', this.form.arrival_date ? this.form.arrival_date : '');
            this.form.form_data.append('comments', this.form.comments);
            this.form.form_data.append('exchange_rate', this.form.exchange_rate);

            await this.$http.post(`/importation`, this.form.form_data)
                .then(response => {
                    ...
                    if (response.data.success) {
                        ...
                        this.$message({
                            showClose: true,
                            message: `Importación registrada con import order task: ${this.form.import_order_task}`,
                            duration: 3000,
                            type: "success"
                        });
        ...
        editInvoice(anInvoiceId) {
            this.form.purchase_order_id = anInvoiceId;
        ...
        processImportCosts(row) {
            ...
            this.form.form_data.append('exchange_rate', row.exchange_rate);
