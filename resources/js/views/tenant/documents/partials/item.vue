                        ...
                        <div class="form-group">
                            ...
                            <table class="table">
                                ...
                                <tr v-for="(row, index) in form.sub_items"
                                    ...
                                    <td>
                                        <el-input v-model="row.unit_price_value">
											<template v-if="row.currency_type_symbol" slot="prepend">
												{{ row.currency_type_symbol }}
											</template>
										</el-input>
                                    </td>
                                    <td>
...
export default {
    ...
    methods: {
        ...
        parseForm(form_data, item){
            ...
            form_data.item  = {... item};
            form_data.item = this.setExtraFieldOfitem(form_data.item)
            form_data.item_unit_types = {... form_data.item.item_unit_types}
            form_data.unit_price_value = form_data.item.sale_unit_price;
            form_data.sub_items = [];           
            ...
            form_data.has_plastic_bag_taxes = form_data.item.has_plastic_bag_taxes;
            form_data.affectation_igv_type_id = form_data.item.sale_affectation_igv_type_id;
            form_data.quantity = 1;

            form_data.has_isc = form_data.item.has_isc
            form_data.percentage_isc = form_data.item.percentage_isc
            form_data.system_isc_type_id = form_data.item.system_isc_type_id

            form_data.lots_group = form_data.item.lots_group

            if(form_data.item.name_product_pdf && this.config.item_name_pdf_description){
                form_data.name_product_pdf = form_data.item.name_product_pdf;
            }
        ...
        calcUnitPrice(unit_price_value, affectation_igv_type_id, has_igv) {
            let unit_price = unit_price_value;
            if (has_igv === false) {
                if (
                    ...
                    unit_price = unit_price_value * 1.18;
        ...
        async clickAddItem() {
            ...
            let unit_price = this.calcUnitPrice(this.form.unit_price_value, affectation_igv_type_id, this.form.has_igv);
            ...
            this.form.sub_items.forEach(sub_item => {
                let affectation_igv_type_id = sub_item.affectation_igv_type_id
                let unit_price = this.calcUnitPrice(sub_item.unit_price_value, affectation_igv_type_id, sub_item.has_igv);

                sub_item.input_unit_price_value = sub_item.unit_price_value;
                sub_item.unit_price = unit_price;
                sub_item.item.unit_price = unit_price;
                sub_item.item.presentation = {};
                sub_item.affectation_igv_type = _.find(this.affectation_igv_types, {'id': affectation_igv_type_id});
                let sub_row = calculateRowItem(sub_item, this.currencyTypeIdActive, this.exchangeRateSale);
				sub_row.IdLoteSelected = null;
