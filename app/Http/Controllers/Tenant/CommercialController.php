...
class CommercialController extends Controller
{
    ...
    {
        $record = new PersonResource(Person::with(['halls', 'user'])->findOrFail($id));
    ...
    public function store(PersonRequest $request)
    {
        ...
        /**START PERSON HALLS */
		if(isset($request->person_halls)){
			foreach ($request->person_halls as $value) {
				$id = $value['id'];unset($value['id']);
                if(!isset($value['warehouse_id'])) $value['warehouse_id'] = null;
                if(!$value['warehouse_id']){
                    $warehouse = new Warehouse();
                    $warehouse->description = "Almacén ".$value['name'];
                    $warehouse->establishment_id = Establishment::ESTABLISHMENT_CUSTOMER_ID;
                    $warehouse->save();
                    $value['warehouse_id'] = $warehouse->id;
                }
				$person->halls()->updateOrCreate(['id' => $id], $value);
			}
		}
        /**END PERSON HALLS */
    ...
    public function export(Request $request)
    {
        ...
        // Filtrar los registros de Participation basados en el ID
        $records = Participation::where('id', $id)->get();
        
        //user id
        $name = optional(Person::find($records->first()->user_id))->name;

        //$filename = ($type == 'customers') ? 'Reporte_Clientes_':'Reporte_Proveedores_';
        
        $filename ='Maquine_Request_Participation_';

        return (new CommercialExport)
                ->name($name)
                ->records($records)
    ...
    public function destroyPersonHall($id)
    {
        $person_hall = PersonHall::findOrFail($id);
        $warehouse_id = $person_hall->warehouse_id;
        $person_hall->delete();
