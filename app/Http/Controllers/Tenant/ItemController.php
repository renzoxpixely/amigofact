...
use App\Http\Requests\Tenant\RdFileRequest;
...
class ItemController extends Controller
{
    ...
    public function getRecords(Request $request){
        ...
        $records = Item::whereTypeUser()->whereNotIsSet()->whereNullParent();
    ...
    public function tables()
    {
        ...
        $game_names = ItemWarehouse::with(['warehouse'])->whereHas('warehouse', function ($query) {
            $query->where('description', \Modules\Inventory\Models\Warehouse::GAMES_HALL);
        })
    ...
    public function store(ItemRequest $request)
    {
        ...
        $item->warehouse_id = $request->request->get('warehouse_id');
        ...
        /**START SUBITEMS */
        if(!$id && $request->sub_items){
            $sub_items_ids = collect($request->sub_items)->map(function ($sub_item) {return $sub_item['id'];});
            Item::whereIn('id', $sub_items_ids->toArray())->update(['parent_id' => $item->id]);
        }
        /**END SUBITEMS */
                    ...
                    if(!empty($temp_serie)){
                        if($delete == true){
                            ...
                            $temp_serie
                                ...
                                ->setGameId($lot['game_id'] ?? null)
                                ->setMinceturCode($lot['mincetur_code'])
                                ->setState($lot['state'])
                                ->push();
                        }
                    }
    ...
    public function uploadRdFile(RdFileRequest $request)
    {
        $file = $request->file('rd_file');

        $date_now = date('YmdHis');
        $directory = 'public'.DIRECTORY_SEPARATOR.'rd-files'.DIRECTORY_SEPARATOR;
        $filename = Str::slug($request->rd_filename) . '-' . $date_now . '.' . $file->getClientOriginalExtension();

        Storage::put($directory . $filename , file_get_contents($file));

        foreach ($request->rd_lot_ids as $lot_id) {
            $lot = ItemLot::findOrFail($lot_id);

            $lot->rd_file_url = $filename;

            $lot->save();
        }

        $lots = ItemLot::findOrFail($lot_id)->item->lots;
