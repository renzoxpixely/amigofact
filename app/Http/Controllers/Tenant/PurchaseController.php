...
use Illuminate\Support\Facades\Storage;
    ...
    class PurchaseController extends Controller
    {
        ...
        public function createImport($purchase_order_id = null)
        {
            return view('tenant.purchases.import_form', compact('purchase_order_id'));
        }
            ...
            switch ($request->column) {
                ...
                case 'fixed_asset':
                    ...
                    $records = Purchase::where(function($q) use ($request) {
                        if(!is_null($request->value)) $q->where($request->column, $request->value);
                    })->whereTypeUser()
                    ->latest();
        ...
        public function tables()
        {
            ...
            $document_types_invoice = DocumentType::DocumentsActiveToPurchase()->orderByDescription()->get();
...
//                return $items;
        ...
        public function item_tables()
        {
            ...
            $game_names = ItemWarehouse::with(['warehouse'])->whereHas('warehouse', function ($query) {
                $query->where('description', Warehouse::GAMES_HALL);
            })
        ...
        public function store(PurchaseRequest $request)
        {
            ...
            try {
                $purchase = DB::connection('tenant')->transaction(function () use ($data) {
                    //$data = self::convertUploadFiles($data);
                    if(!$data['has_import']) $data['arrival_date'] = null;
                    ...
                    foreach ($data['items'] as $row) {
                        ...
                        $convertedRow = self::convertUploadFileItem($row);
                        $p_item->fill($convertedRow);
                        $lots = $convertedRow['lots'] ?? null;
                        if ($lots != null) {
                            // en compras, se guardan los lotes si existen en el campo item de purchase_items
                            $temp_item = $convertedRow['item'];
                            $temp_item['lots'] = $lots;
                            $p_item->item = $temp_item;
                        }
                        $p_item->purchase_id = $doc->id;

                        // TODO check how we can separate items in different warehouses
                        //$transportation_warehouse = Warehouse::where('description', Warehouse::TRANSPORTATION_WAREHOUSE)->first();
                        //$p_item->warehouse_id = $transportation_warehouse['id'];
                        $p_item->save();

                        foreach ($row['sub_item_ids'] as $sub_item_id) {
                            $purchase_sub_item = new PurchaseItem();
                            $purchase_sub_item->fill($convertedRow);

                            $purchase_sub_item->item_id = $sub_item_id;
                            $purchase_sub_item->purchase_id = $doc->id;

                            $purchase_sub_item->is_subitem = true;
                            $purchase_sub_item->parent_id = $p_item->id;

                            $purchase_sub_item->save();
                        }
                        ...
                        if (array_key_exists('lots', $row)) {
                            ...
                            foreach ($row['lots'] as $lot) {
                                ...
                                // Create one loteable type for the purchases & one for the products
                                $product = Item::findOrFail($row['item_id']);

                                $product->lots()->create([
                                    'date' => $row['enters_warehouse_at'] ?: $lot['date'],
                                    'series' => $lot['series'],
                                    'game_id' => $lot['game_id'] ?? null,
                                    'mincetur_code' => $lot['mincetur_code'] ?? null,
                                    'item_id' => $row['item_id'],
        ...
        public static function convert($inputs)
        {
            ...
            $values = [
                ...
                'group_id' => ($inputs->document_type_id === '01' || $inputs->document_type_id === '15') ? '01' : '02',
                'state_type_id' => '01'
            ];
        ...
        private static function convertUploadFiles($data)
        {
            $data['lading']['invoice'] = self::saveFileImportCost('lading-invoice' ,$data['lading']['invoice']);
            $data['insurace']['invoice'] = self::saveFileImportCost('insurace-invoice' ,$data['insurace']['invoice']);
            $data['import_costs']['commission_serie'] = self::saveFileImportCost('import-commission' ,$data['import_costs']['commission_serie']);
            $data['import_costs']['stow_unstow_serie'] = self::saveFileImportCost('import-stow-unstow' ,$data['import_costs']['stow_unstow_serie']);
            $data['import_costs']['handling_serie'] = self::saveFileImportCost('import-handling' ,$data['import_costs']['handling_serie']);
            $data['import_costs']['transport_serie'] = self::saveFileImportCost('import-transport' ,$data['import_costs']['transport_serie']);
            $data['import_costs']['antidumping_serie'] = self::saveFileImportCost('import-antidumping' ,$data['import_costs']['antidumping_serie']);
            $data['import_costs']['storage_serie'] = self::saveFileImportCost('import-storage' ,$data['import_costs']['storage_serie']);
            $data['import_costs']['dispatch_service_serie'] = self::saveFileImportCost('import-dispatch' ,$data['import_costs']['dispatch_service_serie']);

            $import_costs_others = $data['import_costs']['others'] ?? [];
            foreach ($import_costs_others as $index => $other){
                $data['import_costs']['others'][$index]['serie'] = self::saveFileImportCost('import-others' ,$other['serie']);
            }
        ...
        private static function convertUploadFileItem($data)
        {
            $data['mincetur']['link_pdf'] = self::saveFileImportCost('item-mincetur' ,$data['mincetur']['link_pdf']);
            return $data;
        }

        private static function saveFileImportCost($import_cost, $file_info)
        {
            $time = time();
            $attached = $file_info['attached'] ?? null;
            $temp_path = $file_info['temp_path'] ?? null;
            if(is_null($attached) || is_null($temp_path)) return ['list'=>[],'attached'=>null,'temp_path'=>null];
            $file_name_array = explode('.', $attached);
            $file_name = $import_cost.'-'.$time.'.'.$file_name_array[1];
            $file_content = file_get_contents($temp_path);
            Storage::disk('tenant')->put('purchase_import_costs_attached'.DIRECTORY_SEPARATOR.$file_name, $file_content);
            return ['list'=>[$file_name],'attached'=>null,'temp_path'=>null];
        }
        ...
        public function update(PurchaseRequest $request)
        {
            $purchase = DB::connection('tenant')->transaction(function () use ($request) {
                ...
                $doc->group_id = ($request->document_type_id === '01' || $request->document_type_id === '15') ? '01' : '02';
                $doc->user_id = auth()->id();
                $doc->save();
                ...
                foreach ($request['items'] as $row) {
                    $p_item = new PurchaseItem();
                    $row = self::convertUploadFileItem($row);
                    $p_item->fill($row);
                    $p_item->purchase_id = $doc->id;
                    $p_item->save();
    ...
    public function getPurchasesByNumber(Request $request)
    {
        $invoiceNumbers = $request->request->get('import_order_task');

        return Purchase::where('import_order_task', $invoiceNumbers)->get();
    }
