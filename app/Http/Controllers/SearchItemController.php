...
use Maatwebsite\Excel\Facades\Excel;
    ...
    class SearchItemController extends Controller
    {
        ...
        public static function getAllItemBase(Request $request = null, $service = false, $id = 0)
        {
            ...
            return $item->whereIsActive()->whereNullParent()->orderBy('description');
        }
        ...
        // ARmando
        function exportItemsController() {
...
*/
        ...
        public static function getServiceItemByDescription($description, $currency_type_id)
        {
            $request_item = new Request();
            $request_item->merge(['input' => $description, 'search_by_barcode' => 0]);
            $items_services = self::getServiceItem($request_item, 0);
            $items = self::TransformToModalSaleNote($items_services);
            if($items->count()==0){
                Item::createServiceItem($description, $currency_type_id);
                $items_services = self::getServiceItem($request_item, 0);
                $items = self::TransformToModalSaleNote($items_services);
            }
